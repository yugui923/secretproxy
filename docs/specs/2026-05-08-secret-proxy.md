...
**Request lifecycle.** A client sets `HTTP_PROXY=https://secret-proxy:8443` and sends an absolute-form HTTP request with two extra headers: `Proxy-Secret` (the base64 sealed envelope) and `Proxy-Authorization` (a bearer token whose digest is bound into the seal). The proxy verifies the bearer against the digest, unseals the envelope, checks host/path/method against the sealed allowlists, injects the unsealed credential into the outbound request, strips the proxy headers, and forwards to the upstream over TLS. The response returns unchanged.
...
Forward HTTP proxy reached over TLS. Clients set `HTTP_PROXY=https://secret-proxy:8443` and send absolute-form HTTP — the _target_ URL stays `http://` so the proxy can inject headers; the _proxy_ URL is `https://` so the client→proxy hop is encrypted. (See §4.4 for client-runtime compatibility.)
...
| Header                | Purpose                                                                                                                    |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `Proxy-Secret`        | Required. Base64 sealed secret, optionally `; <json-params>` for runtime overrides (§2.5). May repeat to chain processors. |
...
SDKs that refuse to downgrade to plaintext HTTP are unsupported at v1; a `CONNECT` MITM mode (on-the-fly cert generation) is deferred — see §5.2.

### 3.2 Transport Security

**Client → proxy:** TLS 1.3 only (server-authenticated). The listener is HTTPS-only — there is no plaintext fallback. Cert and key are provisioned via `--tls-cert-file` / `--tls-key-file` (§4.1); platform secret systems mount the PEM files. For local development, `secret-proxy gen-tls-cert` emits a self-signed pair (§4.2). mTLS / client cert auth is out of scope at v1 — bearer tokens carry client identity.
...
**Dependencies.** Third-party: `golang.org/x/crypto/nacl/box`, `golang.org/x/crypto/curve25519`. Stdlib only beyond that: `net/http`, `net/http/httputil` (`ReverseProxy` plus a thin forward-proxy shim — no `goproxy` dep), `crypto/tls`, `crypto/subtle`, `crypto/ed25519` (for `gen-tls-cert`), `log/slog`, `regexp`. No DB, no secret-manager SDK. Single static binary.

**Client compatibility.** HTTPS forward proxies (`HTTP_PROXY=https://...`) are supported natively by curl, Go `net/http`, Node 20+, Python `httpx`/`aiohttp`, and JVM `HttpClient`. Older runtimes may need a sidecar.
...
- **`CONNECT` MITM mode** (§3.1) — adds on-the-fly cert minting + CA distribution; revisit when a vendor SDK refuses `HTTP_PROXY` downgrade.
